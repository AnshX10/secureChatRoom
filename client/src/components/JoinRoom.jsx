...
const JoinRoom = ({
  ...
  const validateEncryptionKey = (key) => {
    ...
    if (len < MIN_ENCRYPTION_KEY_LENGTH || len > MAX_ENCRYPTION_KEY_LENGTH) {
      setErrorMessage?.(
        `Encryption key must be between ${MIN_ENCRYPTION_KEY_LENGTH} and ${MAX_ENCRYPTION_KEY_LENGTH} characters.`,
      );
      return false;
    }
    return true;
  ...
  const handleCreate = () => {
    ...
    const normalizedCapacity = Math.max(
      ...
      Math.min(
        MAX_ROOM_CAPACITY,
        parseInt(roomCapacity, 10) || DEFAULT_ROOM_CAPACITY,
      ),
    );
    createRoom(
      username,
  ...
  const roomIdDisplay = (sharedRoomId || "")
    .toUpperCase()
    ...
    <div className="min-h-[100dvh] bg-[#09090b] text-white flex items-center justify-center p-4 sm:p-6 font-sans selection:bg-zinc-700 selection:text-white">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className={`text-center ${isWaiting ? "mb-6" : "mb-8 sm:mb-12"}`}>
          <div
            className={`flex justify-center ${isWaiting ? "mb-3" : "mb-3 sm:mb-4"}`}
          >
          ...
          {!isWaiting && (
            <p className="text-zinc-600 text-[9px] sm:text-[10px] uppercase tracking-[0.3em] sm:tracking-[0.35em] font-medium">
        ...
        <div className="bg-[#0f0f11] border border-zinc-800/40 rounded-2xl shadow-[0_20px_80px_rgba(0,0,0,0.6)] relative overflow-hidden p-5 sm:p-7 md:p-8">
          ...
          {errorMessage && view !== "join" && (
            <div className="mb-6 bg-red-950/50 border border-red-500/20 text-red-300 px-4 py-3 flex items-start justify-between gap-3 rounded-xl relative z-10">
              <p className="text-xs uppercase tracking-wider flex-1">
                {errorMessage}
              </p>
              <button
                type="button"
              ...
              >
                ...
                >
                  <div className="text-left">
                    <span className="block font-bold text-base sm:text-lg tracking-wide">
                      CREATE FREQUENCY
                    </span>
                    <span className="text-zinc-500 text-[10px] uppercase tracking-[0.2em]">
                      Start a new host
                    </span>
                  </div>
              ...
              >
                <div className="flex items-center gap-3 sm:gap-3.5 mb-6 sm:mb-8">
                  <button
                    ...
                    className="p-2 hover:bg-white/5 rounded-xl transition-all active:scale-90 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <LuArrowLeft size={20} />
                  </button>
                  <h2 className="text-base sm:text-lg font-bold uppercase tracking-[0.15em]">
                    Init Host
                  </h2>
                </div>
                ...
                <div className="space-y-5">
                  <div
                    className={`bg-zinc-900/40 border border-zinc-800/30 rounded-xl px-4 py-3 flex items-center gap-3 transition-colors ${isCreatingRoom ? "opacity-50 pointer-events-none" : "focus-within:border-zinc-600"}`}
                  >
                  ...
                  <div
                    className={`flex items-center justify-between gap-3 pt-2 text-[9px] sm:text-[10px] uppercase tracking-[0.15em] sm:tracking-[0.2em] text-zinc-400 ${isCreatingRoom ? "opacity-50 pointer-events-none" : ""}`}
                  >
                    <div className="flex flex-col gap-1">
                      <span className="text-[10px] font-medium">
                        Require Host Approval
                      </span>
                      <span className="text-[8px] text-zinc-600 normal-case tracking-normal">
                        New agents must be accepted by you
                      </span>
                    </div>
                    <button
                      type="button"
                  ...
                  <button
                    ...
                    disabled={
                      ...
                      !username ||
                      !roomName ||
                      roomPassword.length < MIN_ENCRYPTION_KEY_LENGTH
                    }
                ...
                <div className="space-y-5">
                      ...
                      <div className="bg-zinc-900/40 border border-zinc-800/30 rounded-xl px-4 py-3 focus-within:border-zinc-600 transition-colors">
                        ...
                        <div className="relative">
                          <input
                            ...
                            onChange={(e) =>
                              setSharedRoomId(e.target.value.toUpperCase())
                            }
                            value={sharedRoomId}
                  ...
                  {isWaitingApproval && (
                    <div className="bg-zinc-900/40 border border-zinc-800/30 px-4 py-3 text-[10px] text-zinc-400 uppercase tracking-[0.2em] text-center rounded-xl">
                      Wait — host must accept your join request. Keep this
                      window open.
                    </div>
                  )}
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="relative flex items-center justify-center w-11 h-11 shrink-0">
                    <span
                      className="absolute inline-flex h-full w-full rounded-full bg-white/10 animate-ping"
                      style={{ animationDuration: "2s" }}
                    />
                    <span
                      className="absolute inline-flex h-7 w-7 rounded-full bg-white/5 animate-ping"
                      style={{
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span
                      className={`text-xs flex items-center gap-1.5 transition-colors ${copiedRoomId ? "text-green-400" : "text-zinc-600 group-hover:text-zinc-400"}`}
                    >
                      <LuCopy size={11} />
                  ...
                  <p className="text-xs text-zinc-500 leading-relaxed">
                    Share this code with agents. They'll also need the
                    encryption key you set when creating the room.
                  </p>
                </button>
