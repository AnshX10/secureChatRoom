...
const ChatRoom = ({ socket, username, roomId, roomPassword, isHost, leaveRoom }) => {
  ...
  useEffect(() => {
    // Small delay to allow keyboard height adjustment to finish before scrolling
    setTimeout(() => {
  ...
  return (
    /* h-[100dvh] ensures it fits perfectly even with browser UI bars */
    <div className="flex h-[100dvh] w-full bg-black text-white font-mono selection:bg-white selection:text-black overflow-hidden relative">
             ...
             <div className="p-4 flex-1 overflow-y-auto overflow-x-hidden">
               <div className="border border-zinc-800 p-4 mb-6">
                  ...
                  <div className="flex items-center justify-between">
                     ...
                     <button onClick={() => {navigator.clipboard.writeText(roomId); toast.success("Copied");}} className="p-2 hover:bg-white hover:text-black transition text-zinc-400"><IoMdCopy /></button>
                  </div>
               </div>
                 ...
                 {users.map((user, i) => (
                   <div key={i} className="flex items-center gap-3 p-2 border border-transparent">
                      ...
                      <span className="text-xs uppercase tracking-wide truncate">
                        {user.username} {user.username === username && <span className="text-zinc-600">(YOU)</span>}
                      </span>
                   </div>
                 ))}
               </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-black relative">
        ...
        {/* HEADER */}
        <header className="h-14 sm:h-16 border-b border-zinc-800 flex items-center justify-between px-4 sm:px-6 z-30 bg-black flex-shrink-0">
          <div className="flex items-center gap-3 min-w-0">
             <button onClick={() => setShowUsers(true)} className="md:hidden text-2xl text-zinc-400 hover:text-white transition">
               <IoMdPeople />
             </button>
             <div className="truncate">
                <h1 className="font-bold uppercase tracking-[0.2em] text-[10px] sm:text-xs truncate">Secure Link Active</h1>
                <p className="text-[9px] text-zinc-600 uppercase tracking-widest flex items-center gap-1">
                  <span className="w-1.5 h-1.5 bg-white rounded-full animate-pulse"></span> Frequency: {roomId}
                </p>
             </div>
          </div>
          {isHost && (
            <button onClick={() => socket.emit("close_room", {roomId})} className="text-[8px] sm:text-[10px] border border-zinc-800 text-zinc-500 px-3 py-1.5 uppercase hover:bg-red-600 hover:text-white hover:border-red-600 transition-all shrink-0">
              Destroy Room
            </button>
          )}
        </header>

        {/* MESSAGE LIST: flex-1 and overflow-y-auto are critical for mobile keyboards */}
                      ...
                      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className={`absolute bottom-full mb-2 ${msg.own ? "right-0" : "left-0"} z-50 bg-black border border-white shadow-2xl min-w-[130px]`}>
                        ...
                        {msg.own && (
                          <>
                            ...
                            <button onClick={() => deleteForEveryone(msg.id)} className="w-full text-left px-4 py-3 text-[9px] hover:bg-white hover:text-black text-red-900 flex items-center gap-2 uppercase font-bold transition-colors border-t border-zinc-900">
                ...
                <div className="max-w-[90%] bg-zinc-950 border border-zinc-900 border-dashed p-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-[9px] text-white font-bold flex flex-wrap gap-x-1 uppercase truncate">
                        <span>[</span>
                        {typingUsers.map((u, i) => (
        ...
        {/* INPUT AREA: Using flex-shrink-0 to ensure it never disappears */}
        <footer className="p-3 sm:p-4 border-t border-zinc-900 bg-black relative flex-shrink-0 pb-[max(12px,env(safe-area-inset-bottom))]">
          <div className="flex items-center justify-between mb-2">
            {editingMessageId ? (
                <div className="flex items-center gap-2 text-[8px] text-white uppercase tracking-widest border-l border-white pl-2">
                  <span>Modifying...</span>
            ...
            {selfDestructTime > 0 && !editingMessageId && (
                 <div className="flex items-center gap-2 text-[8px] text-red-600 font-bold uppercase tracking-widest animate-pulse">
                    <IoMdTimer /> Timer: {selfDestructTime/1000}s
                 </div>
            )}
          </div>
          ...
          <div className={`flex items-center border p-1 transition-colors ${editingMessageId ? "border-white" : "border-zinc-900 focus-within:border-zinc-700"}`}>
            ...
            <input
              ...
              placeholder={editingMessageId ? "UPDATE..." : "ENCRYPT MESSAGE..."}
