...
import { 
  ...
  IoMdTimer, IoMdPulse // Added Pulse icon for typing
} from 'react-icons/io';
...
const ChatRoom = ({ socket, username, roomId, roomPassword, isHost, leaveRoom }) => {
  ...
  const handleInputChange = (e) => {
    ...
    }, 2000); // Stop typing indicator after 2 seconds of silence
  };
    ...
    <div className="flex h-screen bg-black text-white font-mono selection:bg-white selection:text-black overflow-hidden">
      ...
      <AnimatePresence>
        {(showUsers || window.innerWidth > 768) && (
          <motion.div initial={{ x: -300 }} animate={{ x: 0 }} exit={{ x: -300 }} className="fixed md:relative z-40 w-72 h-full bg-zinc-950 border-r border-zinc-800 flex flex-col">
             <div className="p-6 border-b border-zinc-800">
                <h2 className="text-xl font-black uppercase tracking-widest flex items-center gap-2"><IoMdLock /> CLASSIFIED</h2>
             </div>
             <div className="p-4 flex-1 overflow-y-auto">
               ...
               {users.map((user, i) => (
                 <div key={i} className="flex items-center gap-3 mb-3">
                    <div className="w-8 h-8 bg-zinc-900 border border-zinc-700 flex items-center justify-center font-bold text-xs">{user.username[0].toUpperCase()}</div>
      ...
      <div className="flex-1 flex flex-col relative bg-black">
        ...
        <main className="flex-1 overflow-y-auto p-4 space-y-6">
            ...
            <motion.div layout key={msg.id || Math.random()} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`flex group relative ${msg.system ? "justify-center" : msg.own ? "justify-end" : "justify-start"}`}>
              {msg.system ? (
                ...
                <div className="flex flex-col max-w-[85%] md:max-w-[60%] relative">
                  <div className={`p-4 relative border transition-all ${msg.deleted ? "bg-transparent border-zinc-800 text-zinc-600 italic" : msg.own ? "bg-white text-black border-white" : "bg-black text-zinc-200 border-zinc-800"}`}>
                    <div className="flex justify-between items-start mb-2">
                        {!msg.own && !msg.deleted && <p className="text-[9px] font-bold text-zinc-500 uppercase tracking-wider">{msg.username}</p>}
                        {msg.timer > 0 && !msg.deleted && <span className="flex items-center gap-1 text-[8px] border border-current px-1 rounded-full opacity-60 ml-auto"><IoMdTimer /> {msg.timer / 1000}s</span>}
                    </div>
                    <p className="text-sm leading-relaxed whitespace-pre-wrap flex items-center gap-2">{msg.deleted && <IoMdTrash />} {msg.message}</p>
                  ...
                  {!msg.deleted && (
                      <div onClick={(e) => { e.stopPropagation(); setActiveMenuId(activeMenuId === msg.id ? null : msg.id); }} className={`absolute -top-3 ${msg.own ? "left-0" : "right-0"} p-1 cursor-pointer text-zinc-500 opacity-0 group-hover:opacity-100 transition-opacity ${activeMenuId === msg.id ? "opacity-100" : ""}`}><IoMdMore size={20} /></div>
                  )}
                  <AnimatePresence>
                    {activeMenuId === msg.id && (
                      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className={`absolute top-full mt-2 ${msg.own ? "right-0" : "left-0"} z-50 bg-black border border-white shadow-xl min-w-[160px]`}>
                        <button onClick={() => deleteForMe(msg.id)} className="w-full text-left px-4 py-3 text-xs hover:bg-white hover:text-black text-zinc-400 flex items-center gap-2 uppercase tracking-wide transition-colors"><IoMdTrash /> Delete (Local)</button>
                        {msg.own && (
                          <>
                            <button onClick={() => startEditing(msg)} className="w-full text-left px-4 py-3 text-xs hover:bg-white hover:text-black text-zinc-400 flex items-center gap-2 uppercase tracking-wide transition-colors border-t border-zinc-900"><IoMdCreate /> Edit</button>
                            <button onClick={() => deleteForEveryone(msg.id)} className="w-full text-left px-4 py-3 text-xs hover:bg-white hover:text-black text-zinc-400 flex items-center gap-2 uppercase tracking-wide transition-colors border-t border-zinc-900"><IoMdTrash /> Delete (All)</button>
                          </>
                        )}
                      </motion.div>
                    )}
          ...
          {/* === NEW: TYING INDICATOR BOX (PHANTOM BUBBLE) === */}
          <AnimatePresence>
            {typingUsers.length > 0 && (
              ...
              >
                <div className="max-w-[85%] md:max-w-[60%] bg-zinc-950 border border-zinc-800 p-4 border-dashed animate-pulse">
                  <div className="flex items-center gap-3">
                    <div className="flex items-center justify-center w-6 h-6 border border-zinc-700">
                       <IoMdPulse className="text-white animate-spin text-xs" style={{ animationDuration: '3s' }} />
                    </div>
                    <div>
                      <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-[0.2em] mb-1">Incoming Transmission...</p>
                      <div className="text-xs text-white font-bold flex flex-wrap gap-x-2">
        ...
        {/* Footer / Input (Modified handleInputChange) */}
        <footer className="p-4 border-t border-zinc-800 bg-black relative">
          ...
          <div className={`flex items-center border p-1 transition-colors ${editingMessageId ? "border-white" : "border-zinc-800 focus-within:border-zinc-500"}`}>
            <div className="relative">
                <button onClick={(e) => { e.stopPropagation(); setShowTimerMenu(!showTimerMenu); }} className={`p-3 transition-colors ${selfDestructTime > 0 ? "text-red-500" : "text-zinc-500 hover:text-white"}`}><IoMdTimer size={18} /></button>
                <AnimatePresence>
                    {showTimerMenu && (
                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute bottom-full left-0 mb-2 bg-black border border-zinc-700 shadow-2xl z-50 w-32">
                            {timerOptions.map((opt) => (
                                <button key={opt.label} onClick={() => { setSelfDestructTime(opt.value); setShowTimerMenu(false); }} className={`w-full text-left px-4 py-3 text-[10px] font-bold uppercase tracking-widest hover:bg-white hover:text-black transition-colors ${selfDestructTime === opt.value ? "bg-white text-black" : "text-zinc-400"}`}>{opt.label}</button>
                            ))}
                        </motion.div>
                    )}
                </AnimatePresence>
            ...
            />
            <button onClick={sendMessage} className={`p-3 text-black transition-all ${editingMessageId ? "bg-white" : "bg-white hover:bg-zinc-200"}`}>{editingMessageId ? <IoMdCreate size={18} /> : <IoMdSend size={18} />}</button>
          </div>
        </footer>
      </div>
    </div>
